...
class LoadRPHCall {
    ...
    suspend fun events(parameters: EventQueryParameters = EventQueryParameters()) =
        Clients.get<Page<Event<Store>>>("$workersDev/api/v2/events/?${parameters.toUrl}")

    suspend fun event(
        id: Long,
    ) = Clients.get<Event<StoreFull>>("$rph/events/$id/")

    suspend fun eventRegistrations(
        id: Long,
        parameters: EventRegistrationsQueryParameters = EventRegistrationsQueryParameters()
    ) = Clients.get<Page<EventRegistration>>("$rph/events/$id/registrations/?${parameters.toUrl}")
